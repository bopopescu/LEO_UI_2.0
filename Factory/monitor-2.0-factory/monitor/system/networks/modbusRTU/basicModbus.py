...
class modbus():
    ...
    def __init__(self, port, mode=MODE_RTU, baudrate=BAUDRATE, parity=PARITY, bytesize=BYTESIZE, stopbits=STOPBITS, timeout=TIMEOUT):
    ...
    def __repr__(self):
        ...
        return "{}.{}<id=0x{:x}, mode={}, precalculate_read_size={}, debug={}, serial={}>".format(
            self.__module__,
    ...
    def write_register(self, registeraddress, value):
        """Write integer to 16-bit register in the subordinate.

        The subordinate register can hold integer values in the range 0 to 65535 ("Unsigned INT16").

        Uses Modbus function code 6.

        Only one number will be written.

        Args:
            * registeraddress (int): The subordinate register start address (use decimal numbers, not hex).
            * value (int): The values to store in the subordinate registers.
        ...
        """
        self._genericCommand(6, registeraddress, value, numberOfRegisters=1, payloadformat='register')
    ...
    def _communicate(self, message, number_of_bytes_to_read):
        ...
        time_since_read         = time.time() - self.latest_read_time
        ...
        self.latest_read_time = time.time()
        ...
        if self.debug:
            ...
            text = template.format(
                answer,
                len(answer),
                (self.latest_read_time - latest_write_time) * _SECONDS_TO_MILLISECONDS,
