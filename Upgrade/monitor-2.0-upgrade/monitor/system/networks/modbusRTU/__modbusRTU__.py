...
networkType = networkConstants.networkModbusRTU
networkTypeName = networkConstants.networkModbusRTUText
...
class Network(networkObject.NetworkObject):
  ...
  def run(self):
    ...
    try:
      ...
      log.info( strDebug )
      connection = basicModbus.modbus(self.port, baudrate=self.baudrate, parity=self.parity, bytesize=self.bytesize, stopbits=self.stopbits, timeout=self.timeout, mode=basicModbus.MODE_RTU)
      ...
      if networkTrans is None:
        ...
        if connection is None:
          ...
          for transaction in networkTrans.transactions:
            ...
            elif isinstance(request, modbusConstants.readCoilsRegisters):
              response = modbusConstants.readCoilsRegistersResponse()
              try:
                connection.address = int(networkTrans.networkAddress)
                response.registers = connection.read_bit(request.startingRegister,1)
              except:
                networkTrans.online = False
                networkTrans.offlineMessage = "Network:{0}, Device at address:{1} not responding.  Check address or cabling.".format( self.port, connection.address )
                break

            elif isinstance(request, modbusConstants.readDiscreteInputRegisters):
              response = modbusConstants.readDiscreteInputRegistersResponse()
              try:
                connection.address = int(networkTrans.networkAddress)
                response.registers = connection.read_bit(request.startingRegister)
              except:
                networkTrans.online = False
                networkTrans.offlineMessage = "Network:{0}, Device at address:{1} not responding.  Check address or cabling.".format( self.port, connection.address )
                break

            elif isinstance(request, modbusConstants.readInputRegisters):
              response = modbusConstants.readInputRegistersResponse()
              try:
                connection.address = int(networkTrans.networkAddress)
                response.registers = connection.read_register(request.startingRegister, request.count,4)
              except:
                networkTrans.online = False
                networkTrans.offlineMessage = "Network:{0}, Device at address:{1} not responding.  Check address or cabling.".format( self.port, connection.address )
                break
            ...
            elif isinstance(request, modbusConstants.writeCoilsRegister):
              # response has nothing to do - so this is blank
              response = modbusConstants.writeCoilsRegisterResponse()
              try:
                connection.address = int(networkTrans.networkAddress)
                connection.write_bit(request.register, request.value)
              except:
                networkTrans.online = False
                networkTrans.offlineMessage = "Device at address '" + str(connection.address) + "' not responding.  Check address or cabling."
                break
