    ...
    try:
      if (value != "NA"):
        ...
        with open('/etc/ntp.conf','w') as file:
          file.write(filedata)
        strCommand = "sh /opt/monitor/ntp.sh {0} &".format(value)#"sudo /etc/init.d/ntp stop && sudo /usr/sbin/ntpdate "+ value +" && sudo /sbin/hwclock --systohc && sudo /etc/init.d/ntp start"
        ...
        with open('/etc/ntp.conf','w') as file:
          file.write(filedata)
        strCommand = "sh /opt/monitor/ntp.sh {0} &".format("ntp.ubuntu.com") #" sudo /etc/init.d/ntp stop && sudo /usr/sbin/ntpdate ntp.ubuntu.com && sudo /sbin/hwclock --systohc && sudo /etc/init.d/ntp start"
        subprocess.call(strCommand, shell=True)
    except Exception as e:
      log.debug("There is an exception inside setTimeServerValue")
    finally:
      return True
